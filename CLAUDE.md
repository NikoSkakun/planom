...
Routine extends AppItem (lib/src/models/routine.dart)
  ...
  weekdays?: List<int>               ← specific_days "Week" mode: 0=Mon … 6=Sun, ≥1 day
  monthdays?: List<int>             ← specific_days "Month" mode: 1…31, ≥1 day;
                                       takes precedence over weekdays. Days absent
                                       in a given month are skipped
  continueAfterCompletion: bool      ← when true, a routine that hit its goal stays
                                       interactive in a "middle" state (between
                                       to-do and done) so progress keeps
                                       accumulating past the goal (no auto-reset)
...
Event (lib/src/models/event.dart)                ← calendar entity
  ...
  recurrence?                        ← JSON-encoded Recurrence rule (repeating
                                       events; expanded per-day in the view)
  isDeleted, deletedDate?
...
Recurrence (lib/src/models/recurrence.dart)
  ...
  occursOn(start, date) → bool       ← is `date` a repeat of a series anchored
                                       at `start` (used to expand recurring
                                       Events onto each repeat day)
  JSON-encoded into Task.recurrence / Event.recurrence
...
- Folder/list icons may also be an **emoji / unicode character**, stored as `emoji:<char>` (`isEmojiIconId` / `emojiFromIconId`). `buildFolderItemIcon` renders these as centered text (with `height: 1.0` + even leading distribution so the glyph sits centred in the 22×22 box); they carry no color tint. The icon picker's **Emoji** tab presents a categorised, searchable emoji grid (catalogue + `searchEmojis` keyword search in `lib/src/folders/emoji_catalog.dart`) to set one — free-text/char entry was removed.
- The picker's **Upload** tab opens a dropdown (`showSelectionMenu`) of image sources: Photo Library (`pickIconFromGallery`), Take Photo (`pickIconFromCamera`, mobile only), Choose File (`pickIconFromFile`), and From URL (`downloadIconFromUrl` — fetches over http(s), validates it decodes as an image, saves via `_saveIconBytes`). All land on the shared `icons/<ts>.png` storage. Camera needs `NSCameraUsageDescription` in `ios/Runner/Info.plist`.
...
Single `DatabaseService` class, lazy-opens its DB file (`dbName`, default `planom.db`) via sqflite. Current version: **33**. One `DatabaseService` instance per file — never open the same file with two handles (the default space reuses the global handle; see Spaces).
...
| v33 | `events.recurrence TEXT` — JSON-encoded `Recurrence` rule for repeating events. A recurring event is stored once (anchored at its `date`) and expanded onto each repeat day in the view layer (`EventController.eventsForDate` via `Recurrence.occursOn`) |
...
- `tab_bar_config` / `tab_bar_config:<spaceId>` — the multi-page tab bar layout, **per space**. The default space keeps the unsuffixed key (renaming it would strand every existing layout); a space with no key of its own renders the default space's, and gets one the first time it is edited. `SpaceManager` points `SettingsController` at the active space via `setActiveSpace` on load and on every switch, and drops a deleted space's key through `forgetSpaceSettings`
- `tab_bar_swipe_mode` — `pages` (default) or `spaces`: what a horizontal swipe across the tab bar moves between (see Tab bar). In `spaces` mode the swipe is interactive — see `lib/src/spaces/space_switch_transition.dart`
...
- `routinesForDate(date)` — routines from their `startFloor` (`startDate ?? creationDate`) onward, filtered by schedule: `daily` always, `specific_days` on its `monthdays` (days-of-month 1…31) when set, else on its `weekdays` (Dart `weekday - 1` → 0=Mon … 6=Sun), `interval` per the rules below. Day arithmetic uses `_epochDay` (UTC midnight) to stay DST-safe. `todayRoutines` = `routinesForDate(now)`.
- `isContinuingOnDate(routine, date)` — a routine that reached its goal but has `continueAfterCompletion` set; the Day / Today / Calendar views render these in a "middle" group (between to-do and fully-done) and keep them interactive so `recordProgress` accumulates past the goal (no auto-reset).
- **Interval scheduling.** Fixed-grid (`waitForCompletion = false`): appears when `(epochDay(day) - epochDay(start)) % intervalDays == 0`. Wait-for-completion: `intervalOccurrences(r)` reconstructs the occurrence sequence from completion history (each completion opens the next occurrence at `completionDate + intervalDays`); the open occurrence shows from its scheduled date **through today** (overdue carry-forward), and `openOccurrenceDate` / `isOverdueOn(r, date)` drive the UI. Only completions **on or after `startFloor(r)`** feed that reconstruction — moving the start date forward restarts the routine, so the occurrence it re-opens is the new start day rather than one inherited from the schedule the user just replaced (otherwise an overdue routine stayed overdue no matter what start date it was given). Completing an overdue occurrence on its original day vs the viewed day is the "record original vs shift" choice (the UI prompts for `achieve_all`).
...
**Space switch animation** (`lib/src/spaces/space_switch_transition.dart`): switching Space re-keys `MyApp`, so the outgoing tree is disposed before the incoming one draws and there is no moment when both exist to animate between. `SpaceSwitchTransition` — mounted in `main.dart` **above** the re-keyed subtree so it outlives the switch — rasterises the outgoing frame off its repaint boundary (`toImageSync`, no async gap) and slides that picture out while the new space slides in beside it. The tab-bar drag moves the screen as it happens (with resistance, since there is nothing behind to reveal), then either commits past a third of the screen / on a flick, or springs back. Nothing here may depend on `Directionality` or any other inherited widget the app itself provides — the transition sits above `CupertinoApp`.
...
| `routine_creation_view.dart` | Full-screen `CupertinoPageScaffold` pushed on the Routines tab navigator; `showRoutineCreationView()` helper; reused for editing (`existing` param). Sections: name+icon, inline icon picker (preset grid + "choose photo" tile via `pickCustomIcon`), Frequency (segmented `Daily` / `Specific Days` / `Interval`; Specific Days reveals a `Week` / `Month` switch — `Week` shows a Mon-first `_WeekdayPicker`, `Month` shows a `_MonthdayPicker` 1…31 grid — both refuse to deselect the last day; Interval reveals an "Every N days" stepper + a "Wait for completion" switch) with a Start Date row, Goal (segmented + a "Continue tracking after completion" switch + amount/unit/record fields; a "Record manually" switch hides per-tap recording when on), Reminders (add `time` / `spread` / `afterEach` reminders via clock/duration pickers; spread + afterEach are amount-goal only). `_SegmentedRow` labels use an explicit dynamic `CupertinoColors.label` (the control inherits the page's fallback-black `DefaultTextStyle` otherwise → invisible in dark mode). |
| `routines_view.dart` | Tab root with two segments. **Day** segment: a `_DayNavigator` (‹ date ›, future blocked) over the routines for the selected day, ordered to-do → "continue tracking" (middle, still interactive) → completed (sunk to a collapsible section) — tap a row to record progress for *that* day (manual-entry prompts for an amount); overdue interval+wait occurrences show a red "Overdue · date" subtitle and tapping prompts (record on original day vs complete now and shift); swipe-to-delete; long-press → edit/delete. **All** segment: a `ReorderableListView` (long-press a row to drag-reorder via `reorderRoutines`) of every routine with a schedule subtitle + goal; tap → edit. Custom photos render via `RoutineCircleIcon`. |
...
- `date_wheel_picker.dart` — `showDateWheelPicker(context, {initial, minimumDate?, maximumDate?})` returns `Future<DateTime?>` (null = cancelled). The shared Cupertino wheel date sheet, with a **Today** shortcut in the header that snaps the wheels back to the current day; it hides itself while the wheels already sit on today or when today is outside the allowed range. Used by the routine start date and the Finance entry date. (Birth dates and "repeat until" keep their own wheels — jumping to today means nothing there.) The calendar-grid picker's equivalent is the **Today** pill beside the month name in `calendar_date_picker.dart`, which pages the grid back to the current month without touching the selection.
