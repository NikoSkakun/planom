...
- `gcal_email`, `gcal_selected_calendar_ids` (JSON list), `gcal_default_calendar_id`, `gcal_last_sync_at`, `gcal_synctoken_<calendarId>` — Google Calendar connection state owned by `GoogleCalendarController`; **excluded from backup export and preserved on import** via `GoogleCalendarController.isReservedKey`
...
Lets users view their Google Calendar events alongside Planom events and create/edit/delete events in their existing Google calendars. **No-duplication invariant:** a Google event lives only in Google (held in memory + on-disk JSON cache for offline display); a Planom event lives only in SQLite. Neither store ever writes into the other. `CalendarView` and `DayViewSheet` merge the two streams in the view layer.

| File | Purpose |
|------|---------|
| `oauth_config.dart` | OAuth client id constants (`kGoogleIosClientId`, `kGoogleAndroidClientId`, `kGoogleServerClientId`) + `kGoogleCalendarScopes` + `isGoogleSignInConfigured` gate |
| `google_auth_service.dart` | Wraps `google_sign_in` (iOS / Android / macOS). `trySilentSignIn`, `signIn`, `signOut`, `authClient()` |
...
| `remote_event.dart` | `RemoteEvent` model (in-memory only, never persisted), `GoogleCalendarMeta`, `RemoteEventDraft`, Google ↔ Planom field mapping |
...
| `lib/src/calendar/remote_event_detail_view.dart` | Full-screen edit screen for a remote event. Read-only calendars disable the fields and surface an "Open in Google Calendar" button via `url_launcher` |

**Controller lifetime:** instantiated globally in `main.dart` alongside `SettingsController`/`SecurityService` — **NOT** inside `SpaceManager`. The Google connection is shared across every space, just like appearance/font/locale/lock. Passed through `MyApp` → `HomeShell` → `CalendarView` / `DayViewSheet` / `SettingsView` / `EventCreationSheet` with the same constructor-injection pattern every other controller uses.

**Storage of integration state** (all in the global `planom.db` `app_settings` table):
- `gcal_email`                — connected account
- `gcal_selected_calendar_ids` — JSON list of calendar ids to display
- `gcal_default_calendar_id`   — where Planom-created remote events land
- `gcal_last_sync_at`          — ms since epoch
- `gcal_synctoken_<calendarId>` — per-calendar incremental-sync token

Tokens themselves live on the device's secure storage (handled internally by `google_sign_in` — iOS Keychain / Android Keystore). They are never read by Planom.

**Backup exclusion:** `BackupService` filters out every `app_settings` row matching `GoogleCalendarController.isReservedKey` on both export and import (mirrors the existing `auth_*` exclusion pattern). The on-disk cache is regenerated by the next refresh, so it isn't backed up either.

**Sync strategy:** on connect, Planom fetches all calendars + first window of events (6 months back, 18 months forward) and stores a `nextSyncToken` per calendar. Subsequent refreshes are incremental — on `410 Gone` the controller falls back to a full window re-fetch. `refresh()` is called automatically on `load()` (after silent sign-in succeeds), after `setSelectedCalendars`, and from the "Sync now" row in settings.

**Event creation:** `EventCreationSheet` shows a calendar-picker row when the user is signed in and has at least one writable, selected calendar. The picker contains `Planom (local)` plus each selected calendar; the default is the user-configured default calendar. Picking Planom calls `EventController.addEvent` (existing path); picking a Google calendar calls `GoogleCalendarController.createEvent`. No event is ever written to both stores.

**Read-only calendars** (subscribed holidays, shared-view calendars) are surfaced as `RemoteEvent.isReadOnly = true`. The detail view disables the title/note/date/duration fields and only exposes "Open in Google Calendar" (launches `event.htmlLink` via `url_launcher`).

**iOS setup required** to actually sign in (`ios/Runner/Info.plist`): replace the placeholder `GIDClientID` value with the iOS OAuth client ID and the placeholder `CFBundleURLSchemes` entry with the reversed client ID. Also set `kGoogleIosClientId` in `oauth_config.dart`. Until these placeholders are filled in, `isGoogleSignInConfigured` returns false and the settings page shows a "setup required" state — no network calls are attempted.

**Platforms:** `google_sign_in` covers iOS / Android / macOS / Web. Windows and Linux desktop are not supported by this code path and currently fall through to the "setup required" state on those platforms.
